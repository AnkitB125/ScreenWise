...
document.addEventListener('DOMContentLoaded', () => {
    ...
    dataTable = $('#childrenTable').DataTable({
        columnDefs: [
    ...
    if (childForm) {
        childForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (response.ok) {
                    showAlert('Child added successfully!', false);
                    clearForm(childForm);
                    fetchChildren(); // Refresh the table
                } else {
                    showAlert(responseText, true);
                }
            } catch (error) {
    ...
    // Fetch and display children records on load
    fetchChildren();
});
...
// Populate DataTable
function populateDataTable(children) {
